...
The HTTP surface uses familiar OpenAPI-style sections such as `paths` and `components`. Got-Z specific graph semantics live in explicit extension sections such as `xGotzKeyGrammar`, `xGotzSemantics`, `xGotzRuntimeModes`, and `xGotzConformance`.

The existing public exports from `@got-z/api-spec` remain compatible while the contract is introduced. Runtime parsers, generated Markdown, machine-readable JSON contract output, and generated conformance tests will be layered on top in later changes.

## Context

Got-Z's API is more than HTTP paths. The important semantics live in the JSON key grammar used by `/push` and `/pull`. We want one source of truth that can be read by humans, imported by TypeScript code, and used later for generation.
