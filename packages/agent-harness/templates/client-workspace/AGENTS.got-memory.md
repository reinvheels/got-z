...
Before substantive work, read `.got/memory/current.md` to find workspace identity, runtime configuration, and fallback state. Then use `./.got/bin/got-agent-harness runtime status` to check the runtime, `./.got/bin/got-agent-harness runtime start` if it is not reachable, and `./.got/bin/got-agent-harness pull` to query relevant workspace, user, task, decision, procedure, artifact, question, summary, and recent activity context. For architecture, planning, or handoff work, also read `.got/memory/open-questions.md` and `.got/memory/checkpoints.md`.
