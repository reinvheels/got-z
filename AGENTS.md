...
Global project planning lives under `plan/` at the repository root: `plan/VISION.md`, `plan/SCOPE.md`, and `plan/increments/`. Package-specific planning uses the same structure under that package, for example `packages/agent-harness/plan/`. `VISION.md` is long-term direction and boundaries, `SCOPE.md` is the active minimal increment, and `increments/` is completed or archived history. Do not add planning files under `docs/`; root planning artifacts are canonical.
